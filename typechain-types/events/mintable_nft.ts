import type * as EventTypes from '../event-types/mintable_nft';
...
export default class EventsClass {
	...
	public subscribeOnTransferEvent(callback : (event : EventTypes.Transfer) => void) {
		const callbackWrapper = (args: any[], event: any) => {
			...
			callback(handleEventReturn(_event, getEventTypeDescription('Transfer', 'mintable_nft')) as EventTypes.Transfer);
		};
	...
	public subscribeOnApprovalEvent(callback : (event : EventTypes.Approval) => void) {
		const callbackWrapper = (args: any[], event: any) => {
			...
			callback(handleEventReturn(_event, getEventTypeDescription('Approval', 'mintable_nft')) as EventTypes.Approval);
		};
